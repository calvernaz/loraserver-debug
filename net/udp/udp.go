...
func (u *UDPServer) ReadUDPMessages() <- chan interface{} {
	messages := make(chan interface{})

	addr, err := net.ResolveUDPAddr("udp", fmt.Sprintf("0.0.0.0:%d", u.Port))
	if err != nil {
		logrus.Fatalf("Failed to resolve address: %s", err.Error())
	}

	listener, err := net.ListenUDP("udp", addr)
	if err != nil {
		logrus.Fatalf("Failed to start UDP server: %s", err.Error())
	}
	...
	for {
		...
		if err != nil {
			logrus.Printf("Failed to read message: %s", err.Error())
			continue
		}
...
func handleMessage(msg *lora.Message, messages chan interface{}) {
	logrus.Printf("Handling message from %v", msg.SourceAddr)
	...
	err := msg.Ack()
	if err != nil {
		logrus.Printf("Failed to send ACK: %s", err.Error())
	}
}
...
func publishPushMessagePayloads(gatewayEui []byte, payload lora.PushMessagePayload, messages chan interface{}) {
	if payload.Stat != nil {
		...
		if err != nil {
			logrus.Printf("Failed to convert Stat: %s", err.Error())
		} else {
			messages <- stat
		}
	}
	...
	if payload.RXPK != nil {
		for _, rxpk := range payload.RXPK {
			...
			if err != nil {
				logrus.Printf("Failed to convert RXPK: %s", err.Error())
				continue
			}
			messages <- packet
		}
...
func convertRXPK(gatewayEui []byte, rxpk *lora.RXPK) (*lora.RxPacket, error) {
	data, err := rxpk.ParseData()
	if err != nil {
		logrus.Printf("Failed to parse RXPK: %s", err.Error())
		return nil, err
	}

	networkKey, err := getNetworkKey(gatewayEui, data.DevAddr)
	if err != nil {
		logrus.Printf("Failed to get key (gateway %X, node %X): %s", gatewayEui, data.DevAddr, err.Error())
		return nil, err
	}
	...
	if err != nil {
		logrus.Printf("Failed to test integrity: %s", err.Error())
		return nil, err
	}
	if !ok {
	...
	if err != nil {
		logrus.Printf("Error decrypting data: %s", err.Error())
		return nil, err
	}
